...
void rd_array_subscripts()
{
    if (cur_token.token != Y_LSQUARE)
        error_p("rd_array_subscripts err 1 Y_LSQUARE");
    advance();
    Exp();
    if (cur_token.token != Y_RSQUARE)
        error_p("rd_array_subscripts err 2 Y_RSQUARE");
    advance();
    if (cur_token.token == Y_LSQUARE)
    {
        rd_array_subscripts();
...
void PrimaryExp()
{
    if (cur_token.token == Y_LPAR)
    {
        ...
        if (cur_token.token != Y_RPAR)
            error_p("PrimaryExp err 1 Y_RPAR");
        advance();
    }
...
void Stmt()
{
    ...
    else if (cur_token.token == Y_WHILE)
    {
        advance();
        if (cur_token.token != Y_LPAR)
            error_p("Stmt err 5 Y_LPAR");
        advance();
        LOrExp();
        if (cur_token.token != Y_RPAR)
            error_p("Stmt err 5 Y_RPAR");
        advance();
        Stmt();
        return;
    }
    else if (cur_token.token == Y_IF)
    {
        advance();
        if (cur_token.token != Y_LPAR)
            error_p("Stmt err 6 Y_LPAR");
        advance();
        LOrExp();
        if (cur_token.token != Y_RPAR)
            error_p("Stmt err 6 Y_RPAR");
        advance();
    ...
    else if (cur_token.token == Y_BREAK)
    {
        advance();
        if (cur_token.token != Y_SEMICOLON)
            error_p("Stmt err 7 Y_SEMICOLON");
        advance();
        return;
    }
    else if (cur_token.token == Y_CONTINUE)
    {
        advance();
        if (cur_token.token != Y_SEMICOLON)
            error_p("Stmt err 8 Y_SEMICOLON");
        advance();
        return;
    }
    else if (cur_token.token == Y_RETURN)
    {
        advance();
        if (cur_token.token != Y_SEMICOLON)
            error_p("Stmt err 9 Y_SEMICOLON");
        advance();
        return;
    }
    else
    {
        ...
        advance();
        if (cur_token.token != Y_SEMICOLON)
            error_p("Stmt err 3 Y_SEMICOLON");
        advance();
        return;
    }
}
void BlockItems()
{
    while (cur_token.token == Y_ID || cur_token.token == Y_SEMICOLON || cur_token.token == Y_WHILE || cur_token.token == Y_IF || cur_token.token == Y_BREAK || cur_token.token == Y_CONTINUE || cur_token.token == Y_RETURN || cur_token.token == Y_LBRACKET || cur_token.token == Y_ADD || cur_token.token == Y_SUB || cur_token.token == Y_NOT || cur_token.token == Y_LPAR || cur_token.token == num_INT || cur_token.token == num_FLOAT)
    {
        Stmt();
